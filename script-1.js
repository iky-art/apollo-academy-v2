...
async function route() {
  ...
  try {
    if (hash === "/") return await renderHome(container);
    if (hash === "/bab") return await renderBabList(container);
    if (hash.startsWith("/bab/")) return await renderBabDetail(container, hash.split("/")[2]);
    if (hash === "/profile") return await renderProfilePage(container);
    if (hash === "/login") return await renderLoginPage(container);
    ...
    container.innerHTML = `
      <div style="padding:20px;">
        <p class="loading-screen" style="color:var(--pink-text);text-align:left;">
          ⚠️ Terjadi error saat memuat halaman:<br><br>
          <code style="font-size:12px;">${escapeHtml(err.message || String(err))}</code><br><br>
          Kemungkinan penyebab: tabel Supabase belum dibuat/di-seed, atau URL & anon key di script.js belum benar.
        </p>
      </div>`;
  }
}
...
async function renderHome(container) {
  const { data: chapters, error } = await supabase.from("chapters").select("*").order("number");
  if (error) throw new Error(error.message);
  if (!chapters) throw new Error("Data chapters kosong / null — cek apakah tabel sudah di-seed.");
  let progressMap = {};
  let bookmarks = [];
